//=============================================================================
// RPGツクールMZ - LL_MenuScreenCustom.js v1.4.0
//-----------------------------------------------------------------------------
// ルルの教会 (Lulu's Church)
...
(() => {
	...
	Window_MenuHelp.prototype.drawLeftBlock = function() {
		...
		this.resetTextColor();
		this.drawText(eval(leftBlockValue), this.getFontSize() * leftBlockLabel.length, y, oneThirdWidth * 1.5 - this.getFontSize() * leftBlockLabel.length, leftBlockAlign);
	};
	...
	Window_MenuHelp.prototype.drawRightBlock = function() {
		...
		const oneThirdWidth = Math.floor(this.innerWidth / 3);
		this.changeTextColor(this.systemColor());
		this.drawText(rightBlockLabel, oneThirdWidth * 1.5, y, this.getFontSize() * rightBlockLabel.length, rightBlockAlign);
		this.resetTextColor();
		this.drawText(eval(rightBlockValue), this.getFontSize() * rightBlockLabel.length + oneThirdWidth * 1.5, y, oneThirdWidth * 1.5 - this.getFontSize() * rightBlockLabel.length, rightBlockAlign);
	};
	...
	Window_MenuHelp.prototype.drawCurrency = function() {
		...
		if (currencyWindowPosition == "helpWindowRightBottom") {
			...
			this.drawCurrencyValue($gameParty.gold(), currencyUnit, oneThirdWidth * 1.5 + fontSize * 3, y, oneThirdWidth * 1.5 - fontSize * 3);
		} else {
			// それ以外の場合、独自の項目を描画
			if (!rightBottomBlockLabel) return;
			this.changeTextColor(this.systemColor());
			this.drawText(rightBottomBlockLabel, oneThirdWidth * 1.5, y, this.getFontSize() * rightBottomBlockLabel.length, rightBottomBlockAlign);
			this.resetTextColor();
			this.drawText(eval(rightBottomBlockValue), this.getFontSize() * rightBottomBlockLabel.length + oneThirdWidth * 1.5, y, oneThirdWidth * 1.5 - this.getFontSize() * rightBottomBlockLabel.length, rightBottomBlockAlign);
		}
	};
	...
	Window_MenuHelp.prototype.drawMenuHelp = function() {
		...
		if (leftBottomBlockLabel != "") {
			...
			this.changeTextColor(this.systemColor());
			this.drawText(leftBottomBlockLabel, 0, y, this.getFontSize() * leftBottomBlockLabel.length, leftBottomBlockAlign);
			this.resetTextColor();
			this.drawText(eval(leftBottomBlockValue), this.getFontSize() * leftBottomBlockLabel.length, y, oneThirdWidth * 1.5 - this.getFontSize() * leftBottomBlockLabel.length, leftBottomBlockAlign);
		} else {
			// メニューヘルプを表示
			this.resetTextColor();
