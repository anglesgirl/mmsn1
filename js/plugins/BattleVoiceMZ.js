...
/*:
 * @target MZ
 * @plugindesc [Ver1.15.0]Play voice SE at various situations on battle
...
/*:ja
 * @target MZ
 * @plugindesc [Ver1.15.0]アクターの戦闘時の行動にボイスSEを設定します。
 * @author 神無月サスケ
 * 
 * @param ON switch ID
 * @text ボイス演奏スイッチID
...
(() => {
  ...
  PluginManager.registerCommand(pluginName, 'itemSet', args => {
    ...
    const doesSkipSelf = eval(args.doesSkipSelf);
    if (!args.itemId2) {
      args.itemId2 = "[]";
    }
  ...
  const parameterOfVoice = (name, judgeName, valueName, defaultValue) => {
  ...
  const volumeOfVoice = (name, defaultVolume) => {
    return parameterOfVoice(name, "isVolume", "volume", defaultVolume);
  };

  const pitchOfVoice = (name, defaultPitch) => {
    return parameterOfVoice(name, "isPitch", "pitch", defaultPitch);
  };

  const panOfVoice = (name, defaultPan) => {
    return parameterOfVoice(name, "isPan", "pan", defaultPan);
  };

  const createAudioByFileName = name => {
    const audio = {};
    audio.name = name;
    audio.volume = volumeOfVoice(name, volume);
    audio.pitch = pitchOfVoice(name, pitch);
    audio.pan = panOfVoice(name, pan);
    return audio;
  };
  ...
  const createEnemyAudioByFileName = name => {
    const audio = {};
    audio.name = name;
    audio.volume = volumeOfVoice(name, enemyVolume);
    audio.pitch = pitchOfVoice(name, enemyPitch);
    audio.pan = panOfVoice(name, enemyPan);
    return audio;
  };
  ...
  Game_Actor.prototype.performRecovery = function() {
    ...
    if (this !== subject && this.isActor() == subject.isActor()) {
