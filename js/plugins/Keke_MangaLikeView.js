 ...
 * @default ["{\"名\":\"スクエア\",\"可\":\"true\",\"余白X\":\"8\",\"余白Y\":\"8\",\"不透明度\":\"255\",\"カラー\":\"128, 128, 128, 0.75\",\"縁取りカラー\":\"0, 0, 0, 1\",\"縁取り幅\":\"2\",\"フォーム\":\"スクエア\",\"変形\":\"24\",\"吹き出しツノ\":\"8\",\"位置X\":\"0\",\"位置Y\":\"-8\",\"スキン\":\"{}\"}","{\"名\":\"シャウト\",\"可\":\"true\",\"余白X\":\"8\",\"余白Y\":\"8\",\"不透明度\":\"255\",\"カラー\":\"128, 128, 128, 0.75\",\"縁取りカラー\":\"0, 0, 0, 1\",\"縁取り幅\":\"3\",\"フォーム\":\"シャウト\",\"変形\":\"1\",\"吹き出しツノ\":\"0\",\"位置X\":\"0\",\"位置Y\":\"-8\",\"スキン\":\"{}\"}","{\"名\":\"ダイヤ\",\"可\":\"true\",\"余白X\":\"32\",\"余白Y\":\"16\",\"不透明度\":\"255\",\"カラー\":\"128, 128, 128, 0.75\",\"縁取りカラー\":\"0, 0, 0, 1\",\"縁取り幅\":\"3\",\"フォーム\":\"ダイヤ\",\"変形\":\"0\",\"吹き出しツノ\":\"8\",\"位置X\":\"0\",\"位置Y\":\"-8\",\"スキン\":\"{}\"}"]
 *
 * @param ◉小窓スタイル
 *
 * @param 小窓テキストスタイル
 ...
 *
 * @param フォーム
 * @desc ベースの形状。スクエア・シャウト・ダイヤは『変形』で形を調整できる
 * @type select
 * @option スクエア
 * @option 横アーモンド
 * @option 縦アーモンド
 ...
 *
 * @param 変形
 * @desc フォームの変形量。シャウトは0か1か。ダイヤはマイナスが有効
 * @default 0
 *
 * @param 吹き出しツノ
...
(() => {
    ...
    PluginManager.registerCommand(pluginName, "serifSet", args => {
        ...
        for (const type of typeList) {
            ...
            $gameSystem.setEachSerifKe(set_, type);
        }
    });
    ...
    // セリフセットのセット
    Game_System.prototype.setEachSerifKe = function(set_, type) {
        if (!set_[type+"Serifs"].length) { return; }
        if (!this[type+"SerifsKe"]) { this[type+"SerifsKe"] = {}; }
        const key = set_.type + set_.id;
        this[type+"SerifsKe"][key] = set_[type+"Serifs"];
    };
    
    
    // セリフセットの取得
    Game_System.prototype.getEachSerifKe = function(battler, type, id = null) {
        if (!this[type+"SerifsKe"]) { return; }
        // サブジェクトキー取得
        ...
        // セリフリスト取得
        let datas = this[type+"SerifsKe"][key] || [];
        let serif = null;
        // セリフ取得
        for (const data of datas) {
            const ids = data["ID"] || 0;
            if (!ids || strToNumList(ids.toString()).includes(id)) {
    ...
    // 即時セリフの取得
    Game_System.prototype.getInstantSerifKe = function(battler) {
        if (!this._instantSerifsKe) { return []; }
        // サブジェクトキー取得
        const subject = battler._actorId ? "actor" : "enemy";
        const subjeId = battler._actorId ? battler._actorId : battler._enemyId;
        const key = subject + subjeId;
        // セリフ取得
        let serif = this._instantSerifsKe[key];
        if (serif && !Object.keys(serif).length) { serif = null; }
    ...
    // マンガビューの初期化
    Game_Battler.prototype.initMangaViewKe = function() {
    ...
    Sprite_Battler.prototype.update = function() {
        ...
        if (this._battler) {
            // マンガビューの形成
            this.createMangaViewKe();
            // マンガビューの形成3
            this.createMangaView3Ke();
            // マンガビューの移動
            this.moveMangaViewKe();
            // マンガビューのスケール
            this.scaleMangaViewKe();
            // マンガビューのツノ更新
            this.updateMangaViewTsunoKe();
            // マンガビューの消去
            this.deleteMangaViewKe();
        }
    };
    ...
    Scene_Battle.prototype.createStatusWindow  = function() {
        ...
        this._mangaLikeLayerKe = this._windowLayer;
    };
    ...
    function callOpenMangaView() {
        // マンガビューの初期化
        $gameParty.allMembers().forEach(actor => actor.initMangaViewKe());
        // 味方主体を取得
        const party = $gameParty.aliveMembers().filter(actor => $gameSystem.getEachSerifKe(actor, "open"));
        ...
        // 敵主体を取得
        const troop = $gameTroop.aliveMembers().filter(enemy => $gameSystem.getEachSerifKe(enemy, "open"));
    ...
    BattleManager.processVictory = function() {
        ...
        if (!subje || !$gameSystem.getEachSerifKe(subje, "victory")) {
            const party = $gameParty.aliveMembers().filter(actor => $gameSystem.getEachSerifKe(actor, "victory"));
    ...
    BattleManager.processDefeat = function() {
        ...
        if (!subje || !$gameSystem.getEachSerifKe(subje, "victory")) {
            const troop = $gameTroop.aliveMembers().filter(enemy => $gameSystem.getEachSerifKe(enemy, "victory"));
    ...
    function showMangaView(battler, type, cmd = { action:null, wait:0 }) {
        ...
        // セリフセットを取得
        let serifSet = $gameSystem.getEachSerifKe(battler, type, pack.id);
        // 行動時のみ即時セリフを取得
        if (type == "skill" || type == "item") {
            const instant = $gameSystem.getInstantSerifKe(battler);
            if (instant.serif) {
    ...
    // マンガビューの形成
    Sprite_Battler.prototype.createMangaViewKe = function() {
        if (!keke_showBattleView) { return }
        const battler = this._actor || this._enemy;
        if (!battler) { return; }
    ...
    // マンガビューの形成2
    Sprite_Battler.prototype.createMangaView2Ke = function(pack) {
        // イニット
        const battler = this._actor || this._enemy;
        let text = "";
        let textStyle = null;
        ...
        if (!text) { return; }
        // マンガビューの消去
        this.deleteMangaViewKe(true);
        // マンガビューの形成3フラグ
        this._toCreateMangaView3Ke = { text:text, textStyle:textStyle, baseStyle:baseStyle, pack:pack, type:type, item:item, wait:0 }
    };
    
    
    // マンガビューの形成3
    Sprite_Battler.prototype.createMangaView3Ke = function() {
        if (!this._toCreateMangaView3Ke) { return; }
        if (this._toCreateMangaView3Ke.wait) {
            this._toCreateMangaView3Ke.wait--;
            return;
        }
        // 1からデータ受け取り
        const _1 = this._toCreateMangaView3Ke;
        const text = _1.text;
        ...
        const type = _1.type;
        const item = _1.item;
        this._toCreateMangaView3Ke = null;
        // イニット
        const battler = this._actor || this._enemy;
        ...
        if (textStyle["小窓表示"] != "表示しない") {
            if (textStyle["小窓表示"] == "キャラ名") {
                minis.text = this._actor ? this._actor.name() : this._enemy.name();
        ...
        if (asi && asi.faceBaseSprite) {
            ...
            isAsi = true;
            const cnst = $gameTemp._fullAnimeStatusKe;
            layer = cnst._layers[cnst._messageLayer];
        // 通常の位置設定
        } else {
            x = this._homeX + this._offsetX;
            y = this._homeY + this._offsetY - this._frame.height;
        ...
        if (isAsi) {
            ...
            if ($gameSystem.isSideView()) {
                cfgs.tsunoDire = this._enemy  ? "left" : "right";
            } else {
                cfgs.tsunoDire = "left";
            }
            cfgs.forceDownMax = this._frontViewKe ? 0 : null;
            cfgs.oriSpriteX = this.x;
            cfgs.oriSpriteY = this.y;
        }
        ...
        if (skinSprite) {
            ...
            this._mangaViewsKe.push(skinSprite);
            skinSprite._oriScaleX = skinSprite.scale.x;
            skinSprite._oriScaleY = skinSprite.scale.y;
        }
        if (!skinOver) { layer.addChild(baseSprite); } 
        this._mangaViewsKe.push(baseSprite);
        layer.addChild(textSprite);
        this._mangaViewsKe.push(textSprite);
        if (icons.sprite) {
            layer.addChild(icons.sprite);
            this._mangaViewsKe.push(icons.sprite);
        }
        if (tsunoSprite) {
            layer.addChild(tsunoSprite);
            this._mangaViewsKe.push(tsunoSprite);
        }
        if (miniBaseSprite) {
            layer.addChild(miniBaseSprite);
            this._mangaViewsKe.push(miniBaseSprite);
        }
        if (miniTextSprite) {
            layer.addChild(miniTextSprite);
            this._mangaViewsKe.push(miniTextSprite);
        }
        // スケールのセット
        if (keke_showAnime["時間"]) {
            const duraMax = keke_showAnime["時間"];
            this._mangaViewsKe.forEach(sprite => {
        ...
        if (serifSet) {
            ...
            if (serifSet["効果音"]) {
                serifSet["効果音"].forEach(se => {
                    AudioManager.playSe({ name:se["名"], volume:se["音量"] + keke_volumeLump, pitch:se["ピッチ"] + keke_pitchLump, pan:se["位相"] + keke_panLump });
                }, this);
            }
            // フラッシュ
    ...
    // マンガビューの消去
    Sprite_Battler.prototype.deleteMangaViewKe = function(force = null) {
        if (!keke_showBattleView) { return; }
        // マンガビューがないならリターン
        if (!this._mangaViewsKe.length) { return; }
        // ビューが形成中ならリターン
        //const views = this._mangaViewsKe;
        //if (!views[views.length - 1].bitmap.width) { return; }
        // バトラーがいないならリターン
        const battler = this._actor || this._enemy;
        if (!battler) { return; }
        // 強制ならすぐ消去
        if (force) {
            this.deleteMangaView2Ke();
            return;
        }
        // カウントがnullならリターン
        ...
        if (keke_showAnime["時間"]) {
            const duraMax = keke_showAnime["時間"];
            this._mangaViewsKe.forEach(sprite => {
                sprite._scaleKe = { duraMax:duraMax, duration:0, x:0, y:0, spdX:-sprite.scale.x / duraMax, spdY:-sprite.scale.y / duraMax, del:true };
            }, this);
        } else {
            this.deleteMangaView2Ke();
        }
    ...
    // マンガビューの消去2
    Sprite_Battler.prototype.deleteMangaView2Ke = function() {
        if (!keke_showBattleView) { return; }
        // マンガビューがないならリターン
        if (!this._mangaViewsKe.length) { return; }
        // イニット
        ...
        const scene = SceneManager._scene;
        const spriteset = scene._spriteset;
        const layer = asi && asi.faceBaseSprite ? cnst._layers[cnst._messageLayer] : scene._mangaLikeLayerKe;
        // リムーブ
        this._mangaViewsKe.forEach(sprite => {
    ...
    BattleManager.endBattle = function(result) {
        ...
        spriteset.battlerSprites().forEach(sprite => sprite.deleteMangaViewKe(true));
    };*/
    ...
    // マンガビューの移動
    Sprite_Battler.prototype.moveMangaViewKe = function() {
        if (!keke_viewMove) { return; }
        if (!this._mangaViewsKe || !this._mangaViewsKe.length) { return; }
        // イニット
        const battler = this._battler;
        ...
        if (asi && asi.faceBaseSprite) {
            // マンガビューの移動(フルアニメ )
            this.moveMangaViewFullAnimeKe(asi, gDownMax, gLeftMax, gRightMax, gUpMax,  nowType);
        } else {
            // マンガビューの移動(通常)
            this.moveMangaViewNormalKe(gDownMax, gLeftMax, gRightMax, gUpMax);
        }
    };
    
    
    // マンガビューの移動(フルアニメ)
    Sprite_Battler.prototype.moveMangaViewFullAnimeKe = function(asi, gDownMax, gLeftMax, gRightMax, gUpMax,  nowType) {
        // イニット
        const views = this._mangaViewsKe;
        ...
        // 初期化
        if (this._mangaViewPreXKe == null) { this._mangaViewPreXKe = faceBaseSprite.x; }
        if (this._mangaViewPreYKe == null) { this._mangaViewPreYKe = faceBaseSprite.y; }
        // 移動したか判定
        let moveX = faceBaseSprite.x - this._mangaViewPreXKe;
        let moveY = faceBaseSprite.y - this._mangaViewPreYKe;
        // 移動X
        ...
        // 位置を保存
        this._mangaViewPreXKe = faceBaseSprite.x;
        this._mangaViewPreYKe = faceBaseSprite.y;
    };
    
    
    // マンガビューの移動(通常)
    Sprite_Battler.prototype.moveMangaViewNormalKe = function(gDownMax, gLeftMax, gRightMax, gUpMax) {
        // イニット
        const views = this._mangaViewsKe;
        const baseSprite = views.filter(sprite => sprite._isBase)[0];
        const gWidth = Graphics.width;
        const gHeight = Graphics.height;
        // 初期化
        if (this._mangaViewPreXKe == null) { this._mangaViewPreXKe = this.x; }
        if (this._mangaViewPreYKe == null) { this._mangaViewPreYKe = this.y; }
        // 移動したか判定
        const moveX = this.x - this._mangaViewPreXKe;
        const moveY = this.y - this._mangaViewPreYKe;
        // 移動X
        if (moveX) {
            views.forEach(sprite => sprite.x = this.x + sprite._relativeX);
            // 画面外に出さない
            const vLeft = this.x + baseSprite._relativeX + baseSprite._edgeLeft;
            if (moveX < 0 && vLeft < gLeftMax) {
                views.forEach(sprite => sprite.x += (gLeftMax - vLeft));
            }
            const vRight = this.x + baseSprite._relativeX + baseSprite._edgeRight;
        ...
        // 移動Y
        if (moveY) {
            views.forEach(sprite => sprite.y = this.y + sprite._relativeY);
            // 画面外に出さない
            const vUp = this.y + baseSprite._relativeY + baseSprite._edgeUp;
            if (moveY < 0 && vUp < gUpMax) {
                views.forEach(sprite => sprite.y += (gUpMax - vUp));
            }
            const vDown = this.y + baseSprite._relativeY + baseSprite._edgeDown;
            if (moveY > 0 && vUp > gDownMax) {
                views.forEach(sprite => sprite.y += (gDownMax - vUp));
            }
        }
        // 位置を保存
        this._mangaViewPreXKe = this.x;
        this._mangaViewPreYKe = this.y;
    };
    
    
    // マンガビューのスケール
    Sprite_Battler.prototype.scaleMangaViewKe = function() {
        if (!keke_showBattleView) { return; }
        // ビューがないならリターン
        if (!this._mangaViewsKe.length) { return; }
        // イニット
        ...
        let del = false;
        // ビュー展開
        this._mangaViewsKe.forEach((sprite, i) => {
            // 拡大がないなら次へ
            ...
            if (scale.duration >= scale.duraMax) {
                ...
                if (scale.del) {
                    layer.removeChild(sprite);
                    this._mangaViewsKe[i] = null;
                }
                // 消去予約
                //if (scale.del) { del = true }
                // 拡大削除
                sprite._scaleKe= null;
            }
        }, this);
        // null を消去
        this._mangaViewsKe = this._mangaViewsKe.filter(sprite => sprite);
        // 消去予約を実行
        //if (del) { this.deleteMangaView2Ke(); }
    };
    
    
    // マンガビューのツノ更新
    Sprite_Battler.prototype.updateMangaViewTsunoKe = function() {
        if (!this._mangaViewsKe || !this._mangaViewsKe.length) { return; }
        // イニット
        const views = this._mangaViewsKe;
        const tsunoSprite = views.filter(sprite => sprite._isTsuno)[0];
        if (!tsunoSprite) { return; }
        const asi = getFullAnimeStatusAsi(this._battler);
        // フルアニメ 
        if (asi && asi.faceBaseSprite) {
        ...
        // 通常
            ...
            if (this.x < tsunoSprite.x) { tsunoSprite.scale.x = 1; }
            if (this.x > tsunoSprite.x) { tsunoSprite.scale.x = -1; }
        }
    };
    ...
    Spriteset_Battle.prototype.makeTargetSprites = function(targets) {
        ...
        targetSprites = targetSprites.filter(sprite => {
           ...
           return sprite.validFrontViewAdaptKeMglv();
        }, this);
    ...
    Sprite_Actor.prototype.updatePosition = function() {
        ...
        // フロントビュー位置の更新
        this.updatePosFrontViewKeMglv();
    };
    
    
    // フロントビュー位置の更新
    Sprite_Actor.prototype.updatePosFrontViewKeMglv = function() {
        if (!this.validFrontViewAdaptKeMglv()) { return; }
        const statusWindow = SceneManager._scene._statusWindow;
        const i = $gameParty.allMembers().indexOf(this._actor);
        const rect = statusWindow.faceRect(i);
        this._homeX = statusWindow.x + rect.x + rect.width / 2;
        this._homeY = statusWindow.y + rect.y;
        this._offsetX = 0;
        this._offsetY = 0;
    };
    ...
    // フロントビュー対応するか
    Sprite_Battler.prototype.validFrontViewAdaptKeMglv = function() {
        const statusWindow = SceneManager._scene._statusWindow;
        return keke_adaptFrontView && !$gameSystem.isSideView() && this._actor && (statusWindow && statusWindow.visible);
    };
    ...
    function createTextBox(text, x, y, textStyle, baseStyle, cfgs = {}, minis = {}, icons = {}, handler) {
        ...
        // ツノに応じてテキストY座標を調整
        //textSprite.y += tsunoOy;
        //textLines.y = textLines.y.map(y => y + tsunoOy);
        //if (iconSprite) { iconSprite.y += tsunoOy; }
        // シャウト時は元の幅に
        baseW = form == "シャウト" ? textW + marginX * 2 + baseOutW : baseW;
        baseH = form == "シャウト" ? textH + marginY * 2 + baseOutW : baseH;
        // スキンの形成
...
        }.bind(this));
    ...
    function createTextSprite(text, x, y,  textStyle, fontData = {}, handler) {
        if  (!text) { return { sprite:new Sprite(), lines:{ x:[0], y:[0], w:[0], h:[0] } }; }
        // イニット
        const oriText = text;
        let lineX = 0;
        let lineY = 0;
    ...
    function scanControlChar(text, fontSize, oriFonts, bitmap = null) {
        ...
        if (match) {
            ...
            text = text.replace("\\fi[" + match[1] + "]", "");
            matched = true;
        }
        // フォントカラー
        match = text.match(/^\\fc\[(\d*,?\s*\d*,?\s*\d*,?\s*\d*\.?\d*)\]/);
        if (match) {
            if (bitmap) { bitmap.textColor = !match[1] ? oriFonts.color : "rgba(" + match[1] + ")"; }
            text = text.replace("\\fc[" + match[1] + "]", "");
            matched = true;
        }
        // 縁取りカラー
        match = text.match(/^\\oc\[(\d*,?\s*\d*,?\s*\d*,?\s*\d*\.?\d*)\]/);
        if (match) {
            if (bitmap) { bitmap.outlineColor = !match[1] ? oriFonts.outColor : "rgba(" + match[1] + ")"; }
            text = text.replace("\\oc[" + match[1] + "]", "");
            matched = true;
        }
        // 縁取り幅
        match = text.match(/^\\ow\[([^\]]*)\]/);
        if (match) {
            ...
            text = text.replace("\\ow[" + match[1] + "]", "");
            matched = true;
        }
        // マッチしなかったら \ だけ消去
        if (!matched) { text = text.replace(/^\\/, ""); }
    ...
    function drawBitmapByStyle(bitmap, x, y, width, height, style, rel = null) {
        ...
        if (form == "スクエア") {
            ...
            shoutOuts = [];
            fillShout(bitmap, x, y, width, height, color, shape);
            if (outH) { strokeShout(bitmap, x, y, width, height, outColor, outH, shape); }
        } else if (form == "ダイヤ") {
    ...
    function cutBitmapByStyle(bitmap, x, y, width, height, style) {
        ...
        if (form == "スクエア") {
            ...
            designShout(bitmap, x, y, width, height, shape);
        } else if (form == "ダイヤ") {
    ...
    // シャウトの塗り潰し
    function fillShout(bitmap, x, y, width, height, color = "rgba(0,0,0,1)", shape = 1) {
        const context = bitmap.context;
        context.save();
        context.fillStyle = color;
        designShout(bitmap, x, y, width, height, shape);
    ...
    // シャウトの線画
    function strokeShout(bitmap, x, y, width, height, color = "rgba(0,0,0,1)", lineW = 1, shape = 1) {
        const context = bitmap.context;
        context.strokeStyle = color;
        context.lineWidth = lineW;
        designShout(bitmap, x, y, width, height, shape);
    ...
    function designShout(bitmap, x, y, width, height, shape = 1) {
        const context = bitmap.context;
        context.beginPath();
        const outs = shoutOuts;
        let rand = null;
        width -= x * 2;
        height -= y * 2;
        const xOut = Math.min(width / 9, 80);
        const yOut = Math.min(height / 6, 80);
        rand = shoutXNum ? shoutXNum : 50 + Math.randomInt(50);
